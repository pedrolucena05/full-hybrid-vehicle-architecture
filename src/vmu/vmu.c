...
int calculateCicleEstimated(SystemState *state) {
    ...
    // Enquanto a velocidade calculada for ≤ à velocidade atual...
    while (localSpeed <= state->speed) {
        // escalo o número de ciclos em ponto flutuante
        double nc = cicles / 3.0;
        // essa é a MESMA curva que você vai usar depois em state->speed
        localSpeed = nc * (1.13 - 0.00162 * nc);

        // limite de segurança TAMBÉM 4× maior
        if (cicles >= 1050)
            break;
...
void calculate_speed(SystemState *state) {
    ...
    if (state->accelerator) {
        ...
        double nc = ciclesQuantity / 3.0;
        state->speed   = nc * (1.13 - 0.00162 * nc);  
...
void vmu_control_engines(void) {
    ...
    // No-fuel, start EV-only transition 
        ...
        dbg_string(system_state, "Fuel empty, starting EV transition cycle                        ");
    ...
    if (iecTransitionActive) {
        ...
        dbg_clear(system_state);
        dbg_string(system_state, "IEC transition in progress                                    ");
        ...
        if (system_state->iecPercentage >= ACT_ALONE) {
            ...
            dbg_clear(system_state);
            dbg_string(system_state, "Car running at combustion only mode                           ");
    ...
    // Gradual transition to EV or hybrid mode 
        ...
        dbg_clear(system_state);
        dbg_string(system_state, "EV transition in progress                                     ");
        ...
        if (speedAtEVRange && fuelNotEmpty) {
            ...
            dbg_clear(system_state);
            dbg_string(system_state, "Hybrid EV transition activated                                ");
    ...
    // Hybrid driving logic when both sources available 
        ...
        dbg_clear(system_state);
        dbg_string(system_state, "Car running at standard hybrid mode (hybrid)                  ");
    ...
    // EV-only when speed below threshold and battery sufficient
    else if (speedAtEVRange && (current_battery > TEN_PERCENT) && !evTransitionActive) {
        
        dbg_clear(system_state);
        dbg_string(system_state, "Car running at standard hybrid mode (EV only)                 ");
    ...
    // Fallback default
        ...
        dbg_clear(system_state);
        dbg_string(system_state, "Default engine command                       ");
    }
    ...
    double rpm_ev = RPM_EV_RATIO*(system_state->evPercentage*((system_state->speed * CIRCUNFERENCE_RATIO) / TIRE_PERIMETER));

    if (rpm_ev > MAX_EV_RPM) {
        system_state->evPercentage  = MAX_EV_RPM / ((current_speed * CIRCUNFERENCE_RATIO) / TIRE_PERIMETER);
        system_state->evPercentage  /= RPM_EV_RATIO; 
...
void display_status(const SystemState *state) {
    ...
    printf("Type `1` for accelerate, `2` for brake, or `0` for none, and press Enter:\n");
    cont++;
}
